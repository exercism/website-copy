...
Although `String.format` and `String.formatted()` are wonderful and powerful methods, they are extremely slow for a problem that only requires simple `String` concatenation with the `+` operator.
